...
if (@strlen($op)<3&&@($op+8)<'A_A') {
	...
	if (preg_match('/[\x00-!\'0-9"`&$.,|^[{_zdxfegavpos\x7F]+/i',$_)||@strlen(count_chars(strtolower($_),0x3))>0xd||@strlen($_)>19) {
        ...
        curl_EXEC($ch);
        print curl_getinfo($ch, CURLINFO_EFFECTIVE_URL);
    }
} else {
    if (@stRLEn($op)<4&&@($op+78)<'A__A') {
        $_=@$_GET[''];
        if ((strtOLOwer(suBSTr($_,-4))==='.php')||(strtOLOwer(suBSTr($_,-4))==='php.')||(stripos($_,"\"")!==FALSE)||(stripos($_,"\x3e")!==FALSE)||(stripos($_,"\x3c")!==FALSE)||(stripos(strtolower($_),"amp")!==FALSE)) {
            dIE($secret);
        } else {
            if (stRIPos($_,"..")!==FalsE) {
                die($secret);
